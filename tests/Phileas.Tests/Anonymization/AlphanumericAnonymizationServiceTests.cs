...
public class AlphanumericAnonymizationServiceTests
{
    ...
    {
        var service = new AlphanumericAnonymizationService(new InMemoryContextService(), new Random(), AnonymizationMethod.Realistic);
        const string token = Token;
        var replacement = service.Anonymize(token);
        Assert.NotNull(replacement);
        Assert.NotEmpty(replacement);
        Assert.NotEqual(token, replacement);
        Assert.Equal(token.Length, replacement.Length);
    }

    [Fact]
    public void UuidReplacement()
    {
        var service = new AlphanumericAnonymizationService(new InMemoryContextService(), new Random(), AnonymizationMethod.Uuid);
        var replacement = service.Anonymize(Token);
        Assert.NotEqual(Token, replacement);
    ...
    public void FromListReplacement()
    {
        ...
        var service = new AlphanumericAnonymizationService(new InMemoryContextService(), new Random(), candidates);
    ...
    {
        var service = new AlphanumericAnonymizationService(new InMemoryContextService());
