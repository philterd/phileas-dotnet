...
public class CreditCardAnonymizationServiceTests
{
    private const string Token = "4111-1111-1111-1111";

    [Fact]
    public void RealisticReplacement()
    {
        var service = new CreditCardAnonymizationService(new InMemoryContextService(), new Random(), AnonymizationMethod.Realistic);
    ...
    {
        var service = new CreditCardAnonymizationService(new InMemoryContextService(), new Random(), AnonymizationMethod.Uuid);
        var replacement = service.Anonymize(Token);
        Assert.NotEqual(Token, replacement);
    ...
    public void FromListReplacement()
    {
        ...
        var service = new CreditCardAnonymizationService(new InMemoryContextService(), new Random(), candidates);
    ...
    {
        var service = new CreditCardAnonymizationService(new InMemoryContextService());
