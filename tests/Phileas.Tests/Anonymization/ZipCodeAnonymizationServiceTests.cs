...
public class ZipCodeAnonymizationServiceTests
{
    private const string Token = "90210";

    [Fact]
    public void RealisticReplacement()
    {
        var service = new ZipCodeAnonymizationService(new InMemoryContextService(), new Random(), AnonymizationMethod.Realistic);
    ...
    {
        var service = new ZipCodeAnonymizationService(new InMemoryContextService(), new Random(), AnonymizationMethod.Uuid);
        var replacement = service.Anonymize(Token);
        Assert.NotEqual(Token, replacement);
    ...
    public void FromListReplacement()
    {
        ...
        var service = new ZipCodeAnonymizationService(new InMemoryContextService(), new Random(), candidates);
    ...
    {
        var service = new ZipCodeAnonymizationService(new InMemoryContextService());
