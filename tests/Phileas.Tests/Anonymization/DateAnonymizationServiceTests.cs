...
public class DateAnonymizationServiceTests
{
    private const string Token = "2023-01-15";

    [Fact]
    public void RealisticReplacement()
    {
        var service = new DateAnonymizationService(new InMemoryContextService(), new Random(), AnonymizationMethod.Realistic);
    ...
    {
        var service = new DateAnonymizationService(new InMemoryContextService(), new Random(), AnonymizationMethod.Uuid);
        var replacement = service.Anonymize(Token);
        Assert.NotEqual(Token, replacement);
    ...
    public void FromListReplacement()
    {
        ...
        var service = new DateAnonymizationService(new InMemoryContextService(), new Random(), candidates);
    ...
    {
        var service = new DateAnonymizationService(new InMemoryContextService());
