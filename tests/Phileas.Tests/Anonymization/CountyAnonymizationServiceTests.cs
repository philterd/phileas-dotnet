...
public class CountyAnonymizationServiceTests
{
    private const string Token = "Orange";

    [Fact]
    public void RealisticReplacement()
    {
        var service = new CountyAnonymizationService(new InMemoryContextService(), new Random(), AnonymizationMethod.Realistic);
    ...
    {
        var service = new CountyAnonymizationService(new InMemoryContextService(), new Random(), AnonymizationMethod.Uuid);
        var replacement = service.Anonymize(Token);
        Assert.NotEqual(Token, replacement);
    ...
    public void FromListReplacement()
    {
        ...
        var service = new CountyAnonymizationService(new InMemoryContextService(), new Random(), candidates);
    ...
    {
        var service = new CountyAnonymizationService(new InMemoryContextService());
