...
/// <summary>
/// Runtime filter strategy for date expression detection. Delegates to <see cref="Phileas.Strategies.StandardFilterStrategy.GetStandardReplacement"/> with <c>FilterType.Date</c>.
/// </summary>
public class DateFilterStrategy : StandardFilterStrategy
{
    ...
    public override Replacement GetReplacement(string context, string token, string[] window, double confidence, string? classification, FilterPattern? filterPattern, Crypto? crypto, Fpe? fpe)
        => GetStandardReplacement(context, token, window, confidence, classification, filterPattern, crypto, fpe, FilterType.Date);
}
