...
/// <summary>
/// Runtime filter strategy for Social Security Number (SSN) detection. Delegates to <see cref="Phileas.Strategies.StandardFilterStrategy.GetStandardReplacement"/> with <c>FilterType.Ssn</c>.
/// </summary>
public class SsnFilterStrategy : StandardFilterStrategy
{
    ...
    public override Replacement GetReplacement(string context, string token, string[] window, double confidence, string? classification, FilterPattern? filterPattern, Crypto? crypto, Fpe? fpe)
        => GetStandardReplacement(context, token, window, confidence, classification, filterPattern, crypto, fpe, FilterType.Ssn);
}
