...
{
    private static readonly Analyzer DateAnalyzer = new(
        new FilterPattern.Builder()
            .WithPattern(@"\b(0?[1-9]|1[012])[\/\-\.](0?[1-9]|[12][0-9]|3[01])[\/\-\.](19|20)?\d{2}\b")
        ...
        new FilterPattern.Builder()
            .WithPattern(
                ...
                RegexOptions.IgnoreCase).WithInitialConfidence(0.90).Build(),
        new FilterPattern.Builder()
            .WithPattern(@"\b(Jan|Feb|Mar|Apr|May|Jun|Jul|Aug|Sep|Oct|Nov|Dec)[.\s]\s*\d{1,2},?\s*\d{4}\b",
                RegexOptions.IgnoreCase).WithInitialConfidence(0.85).Build()
    );
