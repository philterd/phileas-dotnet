...
- **`config.splitting.overlap`** ([#82](https://github.com/philterd/phileas-dotnet/issues/82)). Each piece after the first shares this many characters with the end of the previous one, so an entity straddling a piece boundary is seen whole by the later piece. Spans the overlap duplicates are de-duplicated. Defaults to `0`, matching the redaction policy schema.
- **The filter options the policy model was dropping** ([#84](https://github.com/philterd/phileas-dotnet/issues/84)). Thirteen options defined by the redaction policy schema had no property on the .NET policy model, so a policy carrying them was accepted and the options silently discarded: `creditCard.onlyValidCreditCardNumbers`, `onlyWordBoundaries` and `ignoreWhenInUnixTimestamp`; `emailAddress.onlyStrictMatches` and `onlyValidTLDs`; `ibanCode.allowSpaces` and `onlyValidIBANCodes`; `trackingNumber.ups`, `fedex`, `usps` and `allowSpaces`; `url.requireHttpWwwPrefix`; and `phEyeConfiguration.maxIdleConnections`. All are now bound, default to the schema's defaults, and change what is detected. One exception is recorded in the documentation: `ignoreWhenInUnixTimestamp` is bound and applied but cannot currently change the outcome, because this port detects specific card brands rather than any run of 13 to 16 digits, so no timestamp is a candidate. **Behavior change:** `onlyValidCreditCardNumbers` defaults to `true`, so a card-shaped number failing the Luhn checksum is no longer detected; set it to `false` for the previous behavior. See [Supported Identifiers](docs/supported-identifiers.md).
...
- **Every regular expression now runs under a match budget, and a timeout is reported instead of being silently absorbed** ([#98](https://github.com/philterd/phileas-dotnet/issues/98)). `FilterConfiguration.RegexTimeoutMs` (default `1000`) previously reached only the `sections` and `identifiers` filters; it now applies wherever a pattern runs over document text, including `ignoredPatterns`, which a policy author supplies and which had no budget at all. Patterns built outside a filter use the same default via the new `Phileas.Model.RegexDefaults.MatchTimeout`, and `WithRegexTimeoutMs` now rejects a non-positive value rather than accepting one that either throws deeper in construction or disables the bound. The budget applies per match operation, so it bounds any single pattern rather than the total time to filter a document. **Behavior change:** a pattern that exceeds its budget still contributes no spans, but that outcome is no longer indistinguishable from finding nothing. `TextFilterResult.RegexTimeouts` lists the patterns that gave up, and a non-empty list means part of the input went unsearched, so the filtered text should not be treated as fully redacted. An `ignoredPatterns` entry that times out now fails towards redaction: it no longer suppresses the detection it was being tested against. See [API Reference → Regex match budget](docs/api-reference.md#regex-match-budget).

### Fixed

- **The date strategies act on a day-first date instead of leaving it in the document** ([#115](https://github.com/philterd/phileas-dotnet/issues/115)). `SHIFT` re-parsed the detected text with the invariant culture, which reads a numeric date month first, so `15/01/1990` failed to parse and the token was returned unchanged: the date was detected as PHI and then left in the document. Every unambiguous day-first form was affected (`15/01/1990`, `15-01-1990`, `15.01.1990` and the two-digit-year variants), and day-first is the ordinary written form outside the US, so a non-US deployment using `SHIFT` kept most of the dates it detected. All three date strategies now parse with the format the matching pattern recorded, which is the one thing that says which leading number is the day. `SHIFT` writes the date back with that same format, so the ordering, the separator and a two-digit year are preserved, and `TRUNCATE_TO_YEAR` and `RELATIVE` now produce the year and the interval for a day-first date rather than redacting it. **Behavior change:** a detected date that cannot be parsed, such as `1990-02-31` with `onlyValidDates` off, is now redacted by `SHIFT` rather than returned as it was, which is what the documentation always described. See [Filter Strategies → SHIFT](docs/filter-strategies.md#shift).
- **ISO 8601 and several day-first dates are detected** ([#111](https://github.com/philterd/phileas-dotnet/issues/111)). The numeric patterns all placed the year last, so nothing matched `1990-01-15`: ISO 8601 is the ordinary machine-written date form and appears in exports, logs, database dumps and clinical extracts, and every one of them passed through unredacted. Year-first dates are now detected with `-`, `/` and `.` as the delimiter, including the date part of an ISO 8601 timestamp such as `2024-06-01T09:30:00Z`, which is how a log or an export usually writes it; the time is left in the document, since the date is the part that is PHI. Separately, the day-first month-name pattern accepted only the full month name separated by whitespace, so `15 January 1990` was detected while `15 Jan 1990`, `15-Jan-1990`, `15/Jan/1990` and `15-January-1990` were not; abbreviations and the `-`, `/` and `.` delimiters are now accepted, and the two separators in a date must be the same character so `15-January 1990` is not assembled into one. `SHIFT` preserves each new form, including a zero-padded day, and `TRUNCATE_TO_YEAR` and `RELATIVE` act on all of them. A year-first date takes a zero-padded month and day, as ISO 8601 requires and as the Java filter does, so a version string such as `2020.1.5` does not become a date, and `onlyValidDates` still rejects an impossible one such as `1990-02-31`. See [Supported Identifiers → Date](docs/supported-identifiers.md#date).
- **The `TRUNCATE_TO_YEAR` and `RELATIVE` date strategies are implemented instead of silently redacting** ([#109](https://github.com/philterd/phileas-dotnet/issues/109)). The redaction policy schema names both for date filters and this build had neither, so a policy asking for a year or a relative phrase had the date redacted instead, with nothing reported. `TRUNCATE_TO_YEAR` now replaces a date with its year, and `RELATIVE` with a readable interval from today (`3 months ago`, and `2 years 1 months ago` once a year has passed), matching the Java filter's wording so the ports agree. `futureDates` decides whether a date ahead of today is phrased as `in 4 months` or redacted, and means the same thing under `SHIFT`. **Behavior change:** a date strategy name this build does not implement now raises instead of quietly redacting, so a policy cannot silently get a redaction it did not ask for. Strategy names are now matched without regard to case on every filter, as the Java filters do, so a policy written with any casing behaves the same; the schema's enum is uppercase, so this is leniency toward a hand-written policy rather than a second spelling to document. Filters other than the date filter are unchanged and still fall back to redaction. See [Filter Strategies → RELATIVE](docs/filter-strategies.md#relative).
- **The date `SHIFT` strategy reads the field names the policy schema defines** ([#80](https://github.com/philterd/phileas-dotnet/issues/80)). The strategy bound `days`, `months` and `years`, where the schema, the PhiSQL compiler and the Java port all use `shiftDays`, `shiftMonths` and `shiftYears`, and it matched the strategy name `SHIFT_DATE` where the schema uses `SHIFT`. A compiled policy therefore did not shift a date at all: the name did not match, so it fell through and the date was redacted instead. Both field spellings' behaviour is now reachable, `SHIFT_DATE` is still accepted as an alias so existing policies keep working, and `shiftRandom` and `futureDates` are bound and honoured. `shiftRandom` shifts by one to twenty-nine days forward, one to eleven months forward and one or two years back, matching the Java filter; `futureDates`, off by default, applies a shift in the opposite direction when it would move a past date beyond today. See [Filter Strategies → SHIFT](docs/filter-strategies.md#shift).
- **A `::` in prose or code is no longer reported as an IPv6 address** ([#76](https://github.com/philterd/phileas-dotnet/issues/76)). The compressed IPv6 patterns matched a bare `::`, which is the unspecified address, and had no boundary on either side, so any `::` in a technical document became a span and often a ragged one: `std::vector` produced `d::` and `Foo::Bar` produced `::Ba`. The IPv6 forms are now one alternation that does not accept a bare `::` and that cannot begin or end partway through a token. Every compressed form the issue lists is still detected as a single span covering the whole address, as are the expanded, IPv4-mapped and zoned forms. Digits are ASCII, so a fullwidth-digit run is not read as an address, and the boundary is ASCII too, so an address inside non-Latin text is still found. An IPv4 address with a letter or underscore against it, such as the `1.2.3.4` of `v1.2.3.4`, is now detected where it previously was not, at a lower confidence of 0.70 rather than 0.95, since it is as likely to be a version string; one delimited by punctuation keeps the higher confidence. See [Supported Identifiers → IP Address](docs/supported-identifiers.md#ip-address).
- **A trailing `/`, `&` or `#` is no longer dropped from a URL span** ([#78](https://github.com/philterd/phileas-dotnet/issues/78)). Both URL patterns ended with `\b`, so a match could not end on a non-word character. That kept sentence punctuation out of the span, but it also cut the last character off a URL that legitimately ended in one: `http://example.com/path/` was redacted as `http://example.com/path`, leaving the slash behind. A span now has to end on a character a URL may legitimately end on, which keeps anything else out. Writing it that way rather than listing the punctuation to exclude also covers punctuation outside ASCII, so a URL followed by an ideographic full stop or comma is not extended by it. Punctuation inside a path is unaffected, and a URL ending a sentence still leaves the period in the document. See [Supported Identifiers → URL](docs/supported-identifiers.md#url).
- **An unrecognised split method is a policy error instead of being silently replaced with newline splitting** ([#105](https://github.com/philterd/phileas-dotnet/issues/105)). `SplitFactory` accepted `newline`, `width` and `characters` and quietly substituted newline splitting for anything else, so a policy naming a method that does not exist was split by one it never asked for, with nothing reported. The redaction policy specification's own example, `splitting-overlap.json`, asks for `character` in the singular and was hitting exactly this: on a document without newlines it produced one piece rather than splitting, and the overlap it configured did nothing. `character` is now accepted as an alias, and an unrecognised name raises with the accepted names in the message. The method is resolved whenever splitting is enabled rather than only once a document exceeds the threshold, so a mistake surfaces on the first document. **Behavior change:** a policy naming a method that does not exist, with splitting enabled, now fails when it filters rather than filtering with the wrong splitter. See [Policies → Splitting](docs/policies.md#splitting).
- **Credit cards are detected by shape and validated by issuer, so cards in newer ranges are no longer missed** ([#102](https://github.com/philterd/phileas-dotnet/issues/102)). Issuer prefixes were encoded in the detection pattern, so a card in a range the pattern did not list went undetected rather than merely unvalidated. Mastercard's `2221-2720` range, opened in 2017, was absent, as was UnionPay: valid cards passed through unredacted. Detection now matches any run of 13 to 16 digits and the issuer prefix is checked alongside the Luhn checksum under `onlyValidCreditCardNumbers`, where being out of date costs precision rather than recall. Measured over published test cards for every supported brand and a set of non-card digit runs, recall rises from 0.800 to 1.000 with precision unchanged at 1.000. **Behavior changes:** with `onlyValidCreditCardNumbers` set to `false` the filter now redacts every run of 13 to 16 digits, not only brand-shaped ones, so `ignoreWhenInUnixTimestamp` becomes meaningful and is worth enabling alongside it. And a second pattern that matched any 16 digits written as `NNNN NNNN NNNN NNNN` and validated them by checksum alone has been removed, so a formatted number with no issuer prefix is no longer redacted; previously the same digits unformatted were not redacted either, and validation is now consistent between the two. Cards grouped other than four-by-four, such as an American Express written `NNNN NNNNNN NNNNN`, are detected for the first time. See [Supported Identifiers → Credit Card](docs/supported-identifiers.md#credit-card).
- **Span offsets index into the input when splitting is enabled, and the input's whitespace is preserved** ([#92](https://github.com/philterd/phileas-dotnet/issues/92)). Splitting previously filtered each piece independently and concatenated the results, so `CharacterStart` and `CharacterEnd` described positions in the redacted output rather than the input, and the newlines and runs of spaces the splitter trimmed were replaced by a single separator. Each piece is now located in the original input and the replacements are applied to that input once, so splitting is no longer observable in the result: the same document filtered with and without splitting now produces identical output. **Behavior change:** output that previously came back trimmed and rejoined with `Environment.NewLine` now preserves the input's own whitespace. A whitespace-only document at or over the threshold is returned unchanged rather than as an empty string. This is the .NET occurrence of philterd/phileas#386. See [Policies → Splitting](docs/policies.md#splitting).
