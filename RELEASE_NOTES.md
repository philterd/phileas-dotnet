...
- **The optional `id` label on a filter and on a filter strategy** ([#83](https://github.com/philterd/phileas-dotnet/issues/83)). The redaction policy schema declares it on every filter and on both strategy definitions, so a component can be named in logs and diagnostics, and neither was bound: a policy carrying `id`, or compiled from PhiSQL's `OPTIONS (id = '...')`, was accepted and the label dropped on the way in. It is omitted from a serialized policy when unset, carries no PII, and has no effect on what is detected or redacted. See [Policies → Common Identifier Options](docs/policies.md#common-identifier-options).
- **`config.analysis.spanDisambiguation`** ([#52](https://github.com/philterd/phileas-dotnet/issues/52)). A policy can decline span disambiguation, the context-vector step that resolves which type an ambiguously typed span is. The field is declared by the redaction policy schema and had no property on the .NET policy model, so a policy setting it was accepted and the setting silently dropped. It defaults to `true`, so a policy that does not mention it is unaffected, and the host's setting still governs: a policy can decline disambiguation but cannot turn it on when the host has not enabled it. Declining it does not change whether a value is redacted, only which type a contested span is resolved to. See [Span Disambiguation](docs/span-disambiguation.md#turning-it-off-for-one-policy).
...
- **A graphical bounding box honours `enabled` and `priority`** ([#85](https://github.com/philterd/phileas-dotnet/issues/85)). Both are declared by the redaction policy schema for a bounding box and neither was read when the boxes were drawn, so a box switched off still covered the page and overlapping boxes were drawn in declaration order whatever their priority. A disabled box is now skipped, and boxes are drawn in priority order so a higher-priority one lands on top; boxes sharing a priority keep the order the policy declared. **Behavior change:** a policy carrying `enabled: false` on a box stops covering that region, which is what it asked for. The remaining shared properties already bound and are documented as having no effect on a box, which covers a fixed region rather than matching text. See [PDF Redaction → Graphical bounding boxes](docs/pdf-redaction.md#graphical-bounding-boxes).
- **A policy using the deprecated `identifiers.person` key detects again** ([#87](https://github.com/philterd/phileas-dotnet/issues/87)). The redaction policy schema declares it as a deprecated alias carrying a single PhEye configuration, and the Java port binds it; this one did not, so a legacy policy naming it loaded and ran no filter at all, with nothing to say the key had been skipped. It is now read and folded into `pheyes`, after any entry declared there, and written back as `pheyes` so a policy that goes in with `person` comes out with the canonical key. Also corrected the documented JSON key for the PhEye filter, which was written as `pheye` where the model binds `pheyes`. See [Supported Identifiers → PhEye](docs/supported-identifiers.md#pheye).
