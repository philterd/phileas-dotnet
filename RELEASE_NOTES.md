...
- **The `TRUNCATE_TO_YEAR` and `RELATIVE` date strategies are implemented instead of silently redacting** ([#109](https://github.com/philterd/phileas-dotnet/issues/109)). The redaction policy schema names both for date filters and this build had neither, so a policy asking for a year or a relative phrase had the date redacted instead, with nothing reported. `TRUNCATE_TO_YEAR` now replaces a date with its year, and `RELATIVE` with a readable interval from today (`3 months ago`, and `2 years 1 months ago` once a year has passed), matching the Java filter's wording so the ports agree. `futureDates` decides whether a date ahead of today is phrased as `in 4 months` or redacted, and means the same thing under `SHIFT`. **Behavior change:** a date strategy name this build does not implement now raises instead of quietly redacting, so a policy cannot silently get a redaction it did not ask for. Matching stays case-sensitive, as it already was for every strategy, so a lowercase name now raises where it previously redacted in silence; the schema's enum is uppercase. Filters other than the date filter are unchanged and still fall back to redaction. See [Filter Strategies → RELATIVE](docs/filter-strategies.md#relative).
